...
## Description
Spawns dynamic objects based on their source spawner settings. When using this node within a mode, any objects referenced via an `Object Reference` node must be included into that mode's prefab. While the node can function during standard level scripting without being part of a loaded Forge Mode, it is specifically designed to create instances from existing spawners.
...
<figure><img src="../../../.gitbook/assets/2026-08-08_HaloInfinite-SMab.webp" alt="The image shows a script for cleaning up spawned object instances via custom events."><figcaption></figcaption></figure>

## Properties and Limitations
Because the implementation of this node targets specific key features like AI Spawners and Generic Zones, not all properties are inherited by the created instance.

* **Retained properties:** Physics settings, spawn order, spawn properties, and configurations for Generic Zones or AI spawners.
* **Lost properties:** The dynamic material system (specifically colors) is often missing from the cloned object.

> [!warning]
> You cannot use the [Spawn Mode Object](spawn-mode-object.md) node on an already cloned object; it must be used on an object that was created naturally in the scene.

## Spawners vs. Instances
To understand this node, it is necessary to distinguish between a "spawner" and an "instance":
* **Spawner:** Every dynamic object in Forge acts as a spawner—an internal data set containing properties intended to create an object at a specific transform.
...
* **Using Variables:** To store these clones in an [Object Variable](../variables-advanced/object-variable.md) or [Object List Variable](../variables-advanced/object-list-variable.md), you must use `Get Object Without Refresh` or `Get Object List Without Refresh`. These nodes specifically reference the created object instances. 
  * *Note:* Using a standard `Get Object Variable` node will fetch the original spawner rather than the spawned instance.
...
## Cleanup
Because the deletion of spawned instances is not always reliable, it is sometimes necessary to use a cleanup script. A common method is using an [Async Custom Event](../events-custom/trigger-custom-event-global-async.md) that receives the object instance and deletes it after a short delay.
## Source Data
