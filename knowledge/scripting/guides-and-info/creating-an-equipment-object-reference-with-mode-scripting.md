...
* **Classic Equipment Spawns**: These are kit objects and cannot provide an object reference to a Mode Brain.
* **Clones**: Clones are unable to pick up equipment.
* **Bots**: While bots can pick up equipment, they do not drop it, making it difficult to generate a dropped object reference.
* **Player Death Workarounds**: One method involves granting equipment to a player and then killing them to detect the dropped object using `Get All Objects With Spawn Order (0)`. However, this is unideal for multi-round modes or modes utilizing Team Revive options, as it requires player death and necessitates complex systems to return players to their original positions.

## The Forceful Drop Method

A more effective solution for generating an equipment object reference is to forcefully drop equipment that has been granted to a player. This method is viable on most maps because most core map setups include at least one equipment spawner.
