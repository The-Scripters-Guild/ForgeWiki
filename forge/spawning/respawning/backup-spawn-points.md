...
To overcome this issue, one must block the [Root Points' effective radius](../../nav-mesh/building-nav-mesh/nav-mesh-generation-root-points.md#effective-radius) so they can't spread the Nav mesh data. A universal way is to add a Nav Cutter over each invisible Backup Spawn point.
